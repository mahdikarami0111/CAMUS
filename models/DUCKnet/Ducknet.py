    ...
    def __init__(self, in_channels, out_channels, depth=5, init_features=32, normalization='batch',
        ...
        for d in range(1, depth):
            self.multi_scale_blocks.append(
                nn.Conv2d(init_features * (2 ** d), init_features * (2 ** (d + 1)), kernel_size=2, stride=2, padding=0))

        # encoder downsampling blocks
        self.down_blocks = nn.ModuleList(
            [nn.Conv2d(init_features, init_features * 2, kernel_size=2, stride=2, padding=0)])
        for d in range(1, depth):
            self.down_blocks.append(
                nn.Conv2d(init_features * (2 ** d), init_features * (2 ** (d + 1)), kernel_size=2, stride=2, padding=0))

        # encoder duck blocks
        self.encoder_duck_blocks = nn.ModuleList([DuckConv2dBlock(in_channels, init_features,
                                                                  normalization=normalization,
                                                                  duck_multiplier=duck_multiplier,
                                                                  residual_block_multiplier=residual_block_multiplier)])
        for d in range(1, depth):
            self.encoder_duck_blocks.append(
                DuckConv2dBlock(init_features * (2 ** d), init_features * (2 ** d), normalization=normalization,
                                duck_multiplier=duck_multiplier, residual_block_multiplier=residual_block_multiplier))

        # bottleneck residual block
        self.bottleneck = nn.Sequential(
            ResnetConv2dBlock(init_features * (2 ** depth), init_features * (2 ** depth), kernel_size=3,
                              normalization=normalization, multiplier=residual_block_multiplier),
            ResnetConv2dBlock(init_features * (2 ** depth), init_features * (2 ** depth), kernel_size=3,
                              normalization=normalization, multiplier=residual_block_multiplier),
            ResnetConv2dBlock(init_features * (2 ** depth), init_features * (2 ** (depth - 1)), kernel_size=3,
                              normalization=normalization, multiplier=residual_block_multiplier),
            ResnetConv2dBlock(init_features * (2 ** (depth - 1)), init_features * (2 ** (depth - 1)), kernel_size=3,
                              normalization=normalization, multiplier=residual_block_multiplier)
        )
        ...
        # decoder duck blocks
        self.decoder_duck_blocks = nn.ModuleList([DuckConv2dBlock(init_features, init_features,
                                                                  normalization=normalization,
                                                                  duck_multiplier=duck_multiplier,
                                                                  residual_block_multiplier=residual_block_multiplier)])
        for d in range(1, depth):
            self.decoder_duck_blocks.append(
                DuckConv2dBlock(init_features * (2 ** (d)), init_features * (2 ** (d - 1)), normalization=normalization,
                                duck_multiplier=duck_multiplier, residual_block_multiplier=residual_block_multiplier))
