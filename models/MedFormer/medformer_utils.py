...
from layers import DepthwiseSeparableConv, ConvNormAct, FusedMBConv, MBConv, TransformerBlock, BasicBlock
from einops import rearrange
import pdb


class BidirectionAttention(nn.Module):
    def __init__(self, feat_dim, map_dim, out_dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0., map_size=16,
                 proj_type='depthwise'):
        super(BidirectionAttention, self).__init__()
...
class BasicLayer(nn.Module):
    ...
    No downsample of upsample operation in this layer, they are wrapped in the down_block or up_block
    """
...
class up_block(nn.Module):
    def __init__(self, in_ch, out_ch, conv_num, trans_num, conv_block=BasicBlock,
        ...
        self.trans_blocks = BasicLayer(out_ch, map_dim, out_ch, num_blocks=trans_num, heads=heads, dim_head=dim_head,
                                       norm=norm, act=act, expansion=expansion,
                                       attn_drop=attn_drop, proj_drop=proj_drop, map_size=map_size, proj_type=proj_type)
