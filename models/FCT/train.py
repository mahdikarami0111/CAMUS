...
from models.FCT.FCT import FCT, init_weights
...
def train(cfg, preset_indices=None):
    ...
    bce_loss = torch.nn.modules.loss.BCELoss()
    ...
    for e in tqdm(range(epochs)):
        ...
        for i, sampled_batch in enumerate(valloader):
            ...
            loss_ce = (bce_loss(pred_y[2], y) * 0.57 + bce_loss(pred_y[1], down1) * 0.29 +
                       bce_loss(pred_y[0], down2) * 0.14)
            loss_dice = dice_loss(pred_y[2], y)
            loss = 0.5 * loss_dice + 0.5 * loss_ce
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            if i % 25 == 0 or i == len(valloader) - 1:
                print(f"iteration {i}/{len(valloader) - 1} total loss: {loss}")
        ...
        with torch.no_grad():
            ...
            print(calculate_dice_metric(model, testloader, "cuda", sigmoid=False))
